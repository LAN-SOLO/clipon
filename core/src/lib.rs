...
/// Pasteboard marker types (nspasteboard.org) that ask clipboard managers
/// not to record the current contents — set by password managers and the like.
...
pub fn is_concealed_type(uti: &str) -> bool {
    CONCEALED_TYPES.contains(&uti)
}

/// Whether a copy from the app with `bundle_id` should be recorded, given the
/// user's ignore list. Unknown sources are always captured.
pub fn should_capture_from(bundle_id: Option<&str>, ignored: &[String]) -> bool {
    match bundle_id {
        None => true,
        Some(id) => !ignored.iter().any(|i| i.eq_ignore_ascii_case(id)),
    }
}

/// The application that was frontmost when an item was copied.
...
pub struct ClipItem {
    ...
    pub hash: u64,
    /// Where the latest copy came from (absent in stores written before 0.4).
    #[serde(default)]
...
impl Store {
    pub fn add_text(&mut self, text: &str) -> AddOutcome {
        self.add_text_from(text, detect(text), None)
    }
    ...
    pub fn add_text_as(&mut self, text: &str, detected: Detected) -> AddOutcome {
        self.add_text_from(text, detected, None)
    }

    /// Full form: type and source app set by the caller. On dedupe the
    /// source is updated — the latest copy wins.
    pub fn add_text_from(
        &mut self,
        text: &str,
        detected: Detected,
    ...
    /// Text of all stack items joined in stack order; images are skipped.
    /// None if the stack holds no text.
    pub fn stack_merged_text(&self, sep: &str) -> Option<String> {
...
mod tests {
    ...
    fn search_and_filters() {
        ...
        s.add_image("img1.bin", 42, 800, 600, Some("Screenshot.png"), None);
    ...
    fn image_label_in_preview() {
        let mut s = Store::default();
        s.add_image("a.bin", 1, 800, 600, Some("Foto.jpg"), None);
        s.add_image("b.bin", 2, 800, 600, None, None);
    ...
    fn app(id: &str, name: &str) -> Option<SourceApp> {
        Some(SourceApp {
    ...
    #[test]
    fn source_app_recorded_and_updated_on_dedupe() {
        let mut s = Store::default();
        s.add_text_from("hello", Detected::Plain, app("com.apple.Safari", "Safari"));
        assert_eq!(s.items[0].source_app.as_ref().unwrap().name.as_deref(), Some("Safari"));
        let out = s.add_text_from("hello", Detected::Plain, app("com.apple.TextEdit", "TextEdit"));
        assert!(out.deduped);
        assert_eq!(s.items[0].source_app.as_ref().unwrap().name.as_deref(), Some("TextEdit"));
        // unknown source keeps the previous attribution
        s.add_text("hello");
        assert_eq!(s.items[0].source_app.as_ref().unwrap().name.as_deref(), Some("TextEdit"));
    }

    #[test]
    fn old_store_json_without_source_app_loads() {
        let json = r#"{"items":[{"id":"9f2b2c1e-6a0f-4e0e-9b9e-1c2d3e4f5a6b","kind":"text","text":"x","image_file":null,"preview":"x","chars":1,"detected":"plain","pinned":false,"created_at":"2026-01-01T00:00:00Z","last_copied_at":"2026-01-01T00:00:00Z","times_copied":1,"hash":1}],"snippets":[]}"#;
        let s: Store = serde_json::from_str(json).unwrap();
        assert!(s.items[0].source_app.is_none());
    }

    #[test]
    fn concealed_types_and_ignore_list() {
        assert!(is_concealed_type("org.nspasteboard.ConcealedType"));
        assert!(is_concealed_type("org.nspasteboard.TransientType"));
        assert!(is_concealed_type("org.nspasteboard.AutoGeneratedType"));
        assert!(!is_concealed_type("public.utf8-plain-text"));
        let ignored = vec!["com.1password.1password".to_string()];
        assert!(should_capture_from(None, &ignored));
        assert!(should_capture_from(Some("com.apple.Safari"), &ignored));
        assert!(!should_capture_from(Some("com.1password.1password"), &ignored));
        assert!(!should_capture_from(Some("COM.1Password.1Password"), &ignored));
        assert!(should_capture_from(Some("com.1password.1password"), &[]));
    }

    #[test]
    fn stack_merged_text_in_order_without_images() {
        let mut s = Store::default();
        let a = s.add_text("alpha").id;
        let img = s.add_image("i.bin", 9, 10, 10, None, None).id;
        let b = s.add_text("beta").id;
        assert_eq!(s.stack_merged_text("\n"), None);
        s.stack_push(b);
        s.stack_push(img);
        s.stack_push(a);
        assert_eq!(s.stack_merged_text("\n").as_deref(), Some("beta\nalpha"));
